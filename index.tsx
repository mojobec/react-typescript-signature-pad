...
export default class SignaturePad extends React.Component<ISignaturePadProps, {}> {
    ...
    isValid() {
        ...
        sigData.forEach(line => {
            let xArr = line.map((data) => data.x);
            let yArr = line.map((data) => data.y);
    ...
    private addCurve(curve: any) {
        ...
        var newWidth: any = this.strokeWidth(velocity);
        ...
        return (
            <div id="signature-pad" className="m-signature-pad">
                <div className="m-signature-pad--body">
                    <canvas style={customStyles} ref="cv"></canvas>
                </div>
                <i className="fa fa-remove" onClick={this.clear.bind(this)} ></i>
            </div>
        );
    }
}